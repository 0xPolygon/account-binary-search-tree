...
contract('TimelockController', function (accounts) {
  ...
  beforeEach(async function () {
    // Deploy new timelock
    this.mock = await TimelockController.new(MINDELAY, [proposer], [executor], { from: admin });
  ...
  it('initial state', async function () {
    ...
    expect(
      await Promise.all(
        [PROPOSER_ROLE, CANCELLER_ROLE, EXECUTOR_ROLE].map((role) => this.mock.hasRole(role, proposer)),
      ),
    ).to.be.deep.equal([true, false, false]);

    expect(
      await Promise.all(
        [PROPOSER_ROLE, CANCELLER_ROLE, EXECUTOR_ROLE].map((role) => this.mock.hasRole(role, canceller)),
      ),
    ).to.be.deep.equal([false, true, false]);

    expect(
      await Promise.all(
        [PROPOSER_ROLE, CANCELLER_ROLE, EXECUTOR_ROLE].map((role) => this.mock.hasRole(role, executor)),
      ),
