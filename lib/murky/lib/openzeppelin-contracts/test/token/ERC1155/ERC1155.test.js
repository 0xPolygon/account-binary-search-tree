  ...
  describe('internal functions', function () {
    ...
    describe('_mintBatch', function () {
      ...
      context('with minted batch of tokens', function () {
        beforeEach(async function () {
          ({ logs: this.logs } = await this.token.mintBatch(tokenBatchHolder, tokenBatchIds, mintAmounts, data, {
            from: operator,
          }));
        });
    ...
    describe('_burn', function () {
      it("reverts when burning the zero account's tokens", async function () {
        await expectRevert(this.token.burn(ZERO_ADDRESS, tokenId, mintAmount), 'ERC1155: burn from the zero address');
      });

      it('reverts when burning a non-existent token id', async function () {
        await expectRevert(this.token.burn(tokenHolder, tokenId, mintAmount), 'ERC1155: burn amount exceeds balance');
      });
      ...
      context('with minted-then-burnt tokens', function () {
        beforeEach(async function () {
          ...
          ({ logs: this.logs } = await this.token.burn(tokenHolder, tokenId, burnAmount, { from: operator }));
        });
    ...
    describe('_burnBatch', function () {
      ...
      context('with minted-then-burnt tokens', function () {
        beforeEach(async function () {
          ...
          ({ logs: this.logs } = await this.token.burnBatch(tokenBatchHolder, tokenBatchIds, burnAmounts, {
            from: operator,
          }));
        });
