...
contract('ERC721URIStorage', function (accounts) {
  ...
  describe('token URI', function () {
    ...
    it('reverts when queried for non existent token id', async function () {
      await expectRevert(this.token.tokenURI(nonExistentTokenId), 'ERC721URIStorage: URI query for nonexistent token');
    });
    ...
    it('tokens without URI can be burnt ', async function () {
      ...
      await expectRevert(this.token.tokenURI(firstTokenId), 'ERC721URIStorage: URI query for nonexistent token');
    });
